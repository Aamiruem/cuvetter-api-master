...
import { jobSchema } from '../utils/jobValidation.js';
import { sendJobEmail } from '../utils/otp.js';
...
router.post('/post-job', authenticate, async (req, res) => {
    ...
    try {
        const company = await Company.findById(companyId);
        if (!company || !company.isEmailVerified || !company.isPhoneVerified) {
            return res.status(403).json({ message: 'Company not verified or not found' });
        }
...
A new job has been posted by ${company.companyName}.
